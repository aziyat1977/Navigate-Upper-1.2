...
import { LayoutDashboard, GraduationCap, PenTool, Gamepad2, Layers, BookOpen, Menu, X, Globe, Settings, BrainCircuit, Sun, Moon, Mic, Joystick, Infinity } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ currentMode, setMode, lang, setLang, theme, toggleTheme }) => {
  ...
  const modes = [
    ...
    { id: Mode.GENERATOR, label: 'INFINITE DRILL', icon: Infinity },
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 px-6 py-4 flex items-center justify-between bg-white/70 dark:bg-dark-bg/40 backdrop-blur-md border-b border-gray-200 dark:border-white/5 h-20 transition-colors duration-300">
        <div className="flex items-center gap-6">
          <button 
            ...
            className="p-3 -ml-2 hover:bg-neon-blue/20 rounded border border-transparent hover:border-neon-blue/50 text-neon-blue transition-all duration-300 active:scale-95 group interactive-hover"
        ...
        <div className="flex items-center gap-4">
            ...
            <button 
                onClick={toggleTheme}
                className="p-2 rounded-full border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/10 text-gray-600 dark:text-yellow-300 transition-all duration-300 hover:scale-110 active:rotate-180 interactive-hover"
            >
                {theme === 'dark' ? <Sun size={20} /> : <Moon size={20} className="text-indigo-600" />}
            </button>

            {/* Current Mode Indicator (Desktop Only) */}
            <div className="hidden md:flex items-center gap-4 px-6 py-2 rounded border border-gray-200 dark:border-white/10 bg-white/40 dark:bg-black/40 backdrop-blur hover:border-neon-blue/50 transition-colors duration-300">
      ...
      <div className={`fixed top-0 left-0 h-full w-[360px] bg-white dark:bg-[#020005] border-r border-gray-200 dark:border-neon-blue/30 z-[60] transform transition-transform duration-500 cubic-bezier(0.2, 0.8, 0.2, 1) shadow-2xl dark:shadow-[0_0_50px_rgba(0,243,255,0.1)] ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col h-full relative overflow-hidden dark:bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')]">
          ...
          <div className="p-8 flex justify-between items-center border-b border-gray-100 dark:border-white/10 bg-gradient-to-r from-neon-blue/5 to-transparent">
            ...
            <button 
              onClick={() => setIsOpen(false)}
              className="p-2 text-gray-400 hover:text-gray-900 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/10 rounded transition-colors interactive-hover"
            >
              <X size={24} />
            </button>
          </div>
            ...
            {modes.map((m) => {
              ...
              return (
                <button
                  ...
                  className={`w-full group flex items-center gap-4 px-6 py-5 rounded border transition-all duration-300 relative overflow-hidden interactive-hover ${
                    active 
            ...
            <div className="grid grid-cols-3 gap-3">
              {[Language.EN, Language.RU, Language.UZ].map((l) => (
                <button
                  key={l}
                  onClick={() => setLang(l)}
                  className={`relative px-2 py-3 rounded text-sm font-bold font-display transition-all duration-300 border interactive-hover ${
                    lang === l 
